...
def test_hh_adapter_resolves_country_area_ids_before_searching() -> None:
    ...
    settings = Settings(source_hh_country_names="Kazakhstan,Kyrgyzstan,Uzbekistan,Georgia,Moldova")
...
def test_hh_adapter_turns_forbidden_country_search_into_warning() -> None:
    transport = ForbiddenVacanciesTransport()
    settings = Settings(source_hh_country_names="Kazakhstan")
    adapter = HHAdapter(settings=settings, http_transport=transport)
...
def test_hh_adapter_never_uses_russia_or_belarus_from_config() -> None:
    transport = FixtureTransport()
    settings = Settings(source_hh_country_names="Russia,Belarus,Kazakhstan")
    adapter = HHAdapter(settings=settings, http_transport=transport)
...
def test_hh_adapter_maps_vacancy_fields_to_preview_contract() -> None:
    transport = FixtureTransport()
    adapter = HHAdapter(settings=Settings(source_hh_country_names="Kazakhstan"), http_transport=transport)
