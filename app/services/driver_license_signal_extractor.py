...
_CATEGORY_PATTERN = r"c1e|c1|ce|de|c|d|b"
...
_CATEGORY_SEQUENCE = (
    ...
    rf"(?:\s*(?:{_CATEGORY_CONNECTOR_PATTERN})\s*(?:{_CATEGORY_PATTERN}))*)"
)
...
_CLASS_BEFORE_CATEGORY_RE = re.compile(
    rf"\b(?:fuhrerscheinklasse|fahrerlaubnisklasse|fs\s+(?:klasse|kl)|klasse|kl|kategorie|kat|class|category)"
...
_BARE_MARKED_CATEGORY_RE = re.compile(
    ...
    r"von\s+vorteil|ware\s+ideal|nicht\s+erforderlich|required|mandatory|essential|preferred|desirable|"
...
def _extract_clause_categories(clause: str) -> tuple[set[str], bool]:
    ...
    for match in _BARE_MARKED_CATEGORY_RE.finditer(clause):
        if _LANGUAGE_CONTEXT_RE.search(clause[: match.start()]):
            continue
        categories.update(_CATEGORY_RE.findall(match.group("categories")))
...
def _ordered_categories(categories: set[str]) -> tuple[str, ...]:
    order = ("b", "c1", "c1e", "c", "ce", "d", "de")
    return tuple(category.upper() for category in order if category in categories)
