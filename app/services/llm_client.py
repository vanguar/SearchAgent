...
# «ключевые условия (язык, смены, старт)», и когда источник отдавал обрезанный текст
# (Adzuna режет описание на 500 символах), модель дописывала типичные требования
# профессии из общих знаний. Реальный случай: у курьера DHL в тексте не было ни слова
# про немецкий, а в резюме пользователю появилось «Требуется знание немецкого языка».
# Минимальная длина исходного текста для пересказа. Ниже этого порога у модели
# нет материала и она начинает достраивать вакансию по общим знаниям.
_MIN_SUMMARY_SOURCE_CHARS = 120

_SUMMARIZE_VACANCY_PROMPT = """\
Перескажи на русском 1-2 предложениями ТОЛЬКО то, что написано в тексте вакансии ниже.
